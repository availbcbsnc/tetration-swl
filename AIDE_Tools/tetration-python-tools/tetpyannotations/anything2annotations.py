...
def get_args():
    parser = argparse.ArgumentParser(
        description="Parse csv with ip and/or network for annotations. Multiple ips per row transposed. Duplicate ip context concatenated."
    )
    parser.add_argument("-f", dest="file", help="csv file REQUIRED", required=True)
    parser.add_argument(
        "-i", dest="ip", help="ip, network or cidr REQUIRED", required=True
    )
    parser.add_argument(
        "-m",
        dest="mask",
        help="subnet mask or mask length (24 or 255.255.255.0) discarded OPTIONAL",
        required=False,
    )
    parser.add_argument(
        "-n",
        dest="new",
        help="new field combines short and long (if present 40 chars) multiple SHARED REQUIRED",
        required=True,
    )
    parser.add_argument(
        "-s",
        dest="short",
        help="short field (in:out 255 chars) multiple delimited REQUIRED",
        required=True,
    )
    parser.add_argument(
        "-l",
        dest="long",
        help="long field (in:out 255 chars) multiple delimited OPTIONAL",
        required=False,
    )
    parser.add_argument(
        "-p",
        dest="parent",
        help="parent field (in:out 40 chars) multiple SHARED OPTIONAL",
        required=False,
    )
    parser.add_argument(
        "-d",
        dest="delimited",
        help="delimited fields (in:out 255 chars) multiple delimited OPTIONAL",
        required=False,
    )
    parser.add_argument(
        "-a",
        dest="add",
        help="add new field and value (field1=value,field2=value 255 chars) OPTIONAL",
        required=False,
    )
    parser.add_argument(
        "-k",
        dest="keep",
        help="keep fields (in:out 255 chars) retained OPTIONAL",
        required=False,
    )
    parser.add_argument(
        "-e",
        dest="exclude",
        help="values to exclude (field1=value1,value2;field2=value1,value2) discarded OPTIONAL",
        required=False,
    )
    parser.add_argument(
        "-c", dest="case", help="upper/lower/title OPTIONAL", required=False
    )
    parser.add_argument(
        "-R",
        dest="regex",
        help="replace regex with sub (field=regex:sub,regex:sub;field=regex:sub,regex:sub) OPTIONAL special chars [comma] [colon] [equals] [semicolon]",
        required=False,
    )
    parser.add_argument(
        "-S",
        dest="search",
        help="search source for regex and assign value to dest (source=regex:dest=value,source=regex:dest=value) OPTIONAL special chars [comma] [colon] [equals]",
        required=False,
    )
    parser.add_argument(
        "-V",
        dest="vlookup",
        help="lookup key in external file, replace dest with column index (filename:key;index1=dest1,index2=dest2) OPTIONAL",
        required=False,
    )
    parser.add_argument(
        "-C",
        dest="combine",
        help="assign space delimitted fields to dest (dest1=field1,field2,field3;dest2=field4,field5) OPTIONAL",
        required=False,
    )
    parser.add_argument(
...
def main():
    ...
    if args.exclude is not None:
        ...
        for exclude in args.exclude.split(";"):
            if exclude.split("=")[0] in headers:
                excludes.append(
                    str(headers.index(exclude.split("=")[0]))
                    + "="
                    + exclude.split("=")[1]
                )
            else:
    ...
    if args.search is not None:
        ...
        for regex in args.search.split(","):
            if (
                regex.split("=")[0] in headers
                and regex.split(":")[1].split("=")[0] in headers
            ):
                res.append(
                    [
                        headers.index(regex.split("=")[0]),
                        regex.split(":")[0].split("=")[1],
                        headers.index(regex.split(":")[1].split("=")[0]),
                        regex.split(":")[1].split("=")[1],
                    ]
                )
            else:
    ...
    if args.vlookup is not None:
        ...
        if key in headers:
            ...
            print("\nvlookup")
            with open(
                args.vlookup.split(":")[0], encoding="utf8", errors="ignore"
            ) as file:
            ...
            for row in table:
                for record in lookup_table:
                    if row[key_index] == record[0]:
                        for pair in args.vlookup.split(";")[1].split(","):
                            row[headers.index(pair.split(":")[1])] = record[
                                int(pair.split(":")[0])
                            ]
                        break
        else:
    ...
    for row in table:
        ...
        if (
            row[ip_index].lower() not in nulls
            and row[short_index].lower() not in nulls
            and consider
        ):
                    ...
                    if row[short_index] not in line[short_index].split("\n"):
                        ...
                        if args.long is not None:
                            if "=" not in line[long_index]:
                                line[long_index] = (
                                    line[short_index] + "=" + line[long_index]
                                )
                            line[long_index] = (
                                row[short_index]
                                + "="
    ...
    writer = csv.writer(
        open("annotations_" + args.file.split(".")[0] + ".csv", "w", newline="")
    )
