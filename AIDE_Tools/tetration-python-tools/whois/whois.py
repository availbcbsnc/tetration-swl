        ...
        if ipaddress.ip_network(ip).is_global:
            ...
            if lookup and ipaddress.ip_address(ip) in ipaddress.ip_network('192.0.0.0/24'):
                iana = {
                    ...
                    'description': 'Internet Assigned Numbers Authority (IANA)',
                    'country': 'US',
                    'state': 'CA',
                    'city': 'Los Angeles',
                ...
                iana['filter'] = iana['description']
                filter_names.add(iana['filter'])
                unique.add(iana['cidr'])
                cidrs.append(iana)
                lookup = False
            if lookup:
                lookup_count += 1
                ...
                try:
                    ...
                    print('whois lookup failed for ' + ' ' + ip)
