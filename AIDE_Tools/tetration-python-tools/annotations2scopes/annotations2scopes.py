...
def get_args():
    parser = argparse.ArgumentParser(description="pull annotations, derive scopes and keep in sync (add/delete scopes based on current annotations and scopes)")
    parser.add_argument('-p', dest='parent', help='parent scope (create in UI first)', required=True)
    parser.add_argument('-s', dest='scopes', help='heirarchy of 40 char scope annotations (class:application:environment)', required=True)
    parser.add_argument('-d', dest='descriptions', help='(optional) heirarchy of 255 char scope descriptions (class_description:application_description:environment_decription', required=False)
    parser.add_argument('-c', dest='config', help='(optional) true/false column to trigger config', required=False)
    parser.add_argument('-e', dest='exclude', help='(optional) do not add/delete children for specified scope(s), comma delimited', required=False)
...
def main():
    ...
    annotations = sorted(annotations, key=lambda x: ipaddress.ip_network(x[0]).prefixlen)
        ...
        for i in range(len(scope_indeces)):
            if row[scope_indeces[i]] == '':
                for network in networks:
                    if '/' in row[0] and row[0] != network[0]:
                        if next(ipaddress.ip_network(row[0]).hosts()) in ipaddress.ip_network(network[0]) and network[scope_indeces[i]] != '':
                            row[scope_indeces[i]] = network[scope_indeces[i]]
                    elif '/' not in row[0] and ipaddress.ip_address(row[0]) in ipaddress.ip_network(network[0]):
            ...
            if config:
                ...
                if '::' not in scope and not scope.endswith(':'):
                    ...
                    if args.descriptions is not None:
                        unique_descriptions.add(scope + '\t' + row[description_indeces[i]].replace('\t', ' '))
    ...
    for app_scope in app_scopes:
        if app_scope['name'].split(':')[0] == args.parent.split(':')[0]:
            if app_scope['name'][0:len(args.parent)] == args.parent or args.parent[0:len(app_scope['name'])] == app_scope['name']:
                # if app_scope['name'].split(':')[0] == args.parent.split(':')[0]:
    ...
    print("\nips (total/orphaned/scoped): " + str(ip_count) + '/' + str(len(orphans)) + '/' + str(ip_count - len(orphans)))

    print("\nscopes (current/add/delete/future): " + str(len(current_scopes)) + '/' + str(len(add_scopes)) + '/' + str(len(delete_scopes)) + '/' + str(len(current_scopes) - len(delete_scopes) + len(add_scopes)))
    ...
    if response == 'yes':
        ...
        if len(add_scopes) > 0 or len(delete_scopes) > 0:
            final_scopes = list(set(current_scopes).union(set(add_scopes)) - set(delete_scopes))
    ...
    if len(add_scopes) > 0 or len(delete_scopes) > 0:
        ...
        if response == 'yes':
            ...
            if len(delete_scopes) > 0:
                ...
                if response == 'yes':
                    ...
                    for scope_id in scope_ids:
                        if scope_id[0] in delete_scopes and scope_id[0] not in args.parent:
                            # if scope_id[0] not in scopes and scope_id[0] not in args.parent:
                            commit = True
                            count += 1
                            keystrokes += 2
                            print(str(count) + "/" + str(len(delete_scopes)) + " delete " + scope_id[0].replace('\n', ' '))
                ...
                if response == 'yes':
                    ...
                    for scope in scopes:
                        if scope not in current_scopes:
                            ...
                            short_name = L.pop()
                            parent_name = ":".join(L)
                            header = headers[scope_indeces[len(scope.replace(args.parent + ':', '').split(':')) - 1]]
                            description = ''
                            ...
                            count += 1
                            keystrokes += 2 * len(short_name) + len(description) + len(header) + 4

                            print(str(count) + "/" + str(len(add_scopes)) + " create " + scope.replace('\n', ' '))
            ...
            # show tree
            print("\nips (total/orphaned/scoped): " + str(ip_count) + '/' + str(len(orphans)) + '/' + str(ip_count - len(orphans)))
            print("\nscopes (previous/add/delete/current): " + str(len(current_scopes)) + '/' + str(len(add_scopes)) + '/' + str(len(delete_scopes)) + '/' + str(len(current_scopes) - len(delete_scopes) + len(add_scopes)))
                ...
                for app_scope in app_scopes:
                    if app_scope['name'].split(':')[0] == args.parent.split(':')[0]:
                        ...
                        if len(app_scope['name'].split(':')) == 1:
                            tree.create_node(app_scope['short_name'].replace('\n', ' '), app_scope['id'])
                        else:
                            tree.create_node(app_scope['short_name'].replace('\n', ' '), app_scope['id'], app_scope['parent_app_scope_id'])
                print("=============================== CURRENT ===============================")
                tree.show()
